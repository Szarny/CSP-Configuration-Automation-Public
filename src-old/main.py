...
from Analyzer import HTMLParser, HTMLAnalyzer, JSAnalyzer
...
def main():
    ...
    # HTMLコンテンツをパース
    parsed_html = HTMLParser.parse_html(html_contents)

    # HTMLコンテンツからJavaScriptコンテンツへのリンクを抽出
    js_link_list = js_link_extractor(parsed_html)
    ...
    csp_info_meta_format, csp_info_for_display, csp_info_external_script_integrity = configure_csp(csp_info_db, parsed_html)
